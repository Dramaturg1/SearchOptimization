...
class GradientDescentMethod:
    """
    Класс для метода градиентного спуска
    """

    def __init__(self, view, current_func, current_zmin, current_zmax, point_item):
        self.view = view
    ...
    def set_function(self, func, zmin, zmax):
        """Обновляет текущую функцию"""
        self.current_func = func
        self.current_zmin = zmin
        self.current_zmax = zmax

    def z_to_vis(self, z):
        """Конвертирует Z в визуальные координаты"""
    ...
    def run_multiple(self, start_points, eps_grad, max_iter):
        """Запускает градиентный спуск из нескольких точек"""
        ...
        for x0, y0 in start_points:
            ...
            print(f"Старт ({x0:.2f},{y0:.2f}): минимум найден: x={x:.5f}, y={y:.5f}, f={f:.5f}")
